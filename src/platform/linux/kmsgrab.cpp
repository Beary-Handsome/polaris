  ...
  return DRM_MODE_CONNECTOR_##y
    ...
    public:
      ...
      inline capture_e refresh(file_t *file, egl::surface_descriptor_t *sd, std::optional<std::chrono::steady_clock::time_point> &frame_timestamp) {
        ...
        // A display-topology change (monitor powering off, or the desktop being moved onto
        // a dummy plug / another CRTC — e.g. the SWAP_PRIMARY dongle path) retires the plane
        // we pinned at init(). drmModeGetPlane() then returns NULL, and a plane that still
        // exists but stopped scanning out reports fb_id == 0. Either way the captured CRTC
        // is dead: re-resolve onto the now-active output instead of dereferencing a null
        // plane (crash) or spinning on capture_e::timeout forever (frozen stream).
