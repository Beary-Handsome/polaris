...
std::string get_local_ip_for_gateway() {
  ...
  std::shared_ptr<display_t> display(mem_type_e hwdevice_type, const std::string &display_name, const video::config_t &config) {
    ...
    // session can start the cage runtime afterwards. When the cage is live, capture
    // must target the nested compositor (wlgrab connects to its socket) — never the
    // physical desktop via KMS/X11, which the startup bitset may otherwise select.
    if (config::video.linux_display.use_cage_compositor && cage_display_router::is_running()) {
